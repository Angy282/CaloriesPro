...
function addBudget() {
  ...
  dailyCalories.textContent = `Daily Calories goal: ${budget}`;
  budgetField.value = "";
}
...
async function checkCalories() {
  ...
  const rounded = Math.round(per100);
  ...
  resultItem.textContent = `${counter}. ${capitalize(
    food.description
  )}: ${rounded} kcal for 100g`;
  resultDiv.appendChild(resultItem);
  inputField.value = "";
}

// Localstorage
const savedBudget = localStorage.getItem("dailyCaloriesBudget");
if (savedBudget) {
  dailyCalories.textContent = `Daily Calories: ${savedBudget}`;
}
